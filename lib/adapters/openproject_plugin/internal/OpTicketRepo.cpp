...
Task<Result<std::vector<aid::Ticket>>> OpTicketRepo::getAllPaged(std::string baseUrlWithFilters) {
    // The offset/pageSize/`total` loop itself lives in pagedGet (paging.h) —
    // shared with OpUserRepo, which had the same un-paged-GET defect one level
    // up. This wrapper only supplies the per-page parse.
    //
    // `baseUrlWithFilters` already carries `?filters=<…>`, so pagedGet's
    // pagination params splice on with `&`. No `sortBy`: these URLs are
    // live-verified as they stand and OpDashboardBuilder re-sorts the merged
    // result anyway.
    std::vector<aid::Ticket> all;
    // The sink borrows `all` and the two maps. It runs synchronously inside
    // pagedGet while THIS coroutine is suspended at the co_await, so the frame
    // holding `all` is alive for every invocation.
    auto sunk = co_await pagedGet(http_, std::move(baseUrlWithFilters), kDashboardPageSize,
                                  [this, &all](const nlohmann::json& elements) -> Result<void> {
                                      all.reserve(all.size() + elements.size());
                                      for (const auto& e : elements) {
                                          auto parsed = parseFromHal(e, fieldMap_, statusMap_);
                                          if (!parsed)
                                              return unexpected(parsed.error());
...
OpTicketRepo::findCallTicketsInProjectsOpen(const std::vector<aid::ProjectId>& projects) {
    ...
    // DOCUMENTED CEILING: every project id the viewer belongs to goes into ONE
    // `filters=[{"project":{"operator":"=","values":[…]}}]` query string. Since
    // OpUserRepo::projectsForUser became properly paged, that list is no longer
    // capped at 20 — so the URL length is now the binding constraint. Each id
    // costs ~12 encoded chars (`,"123"` → `%2C%22123%22`), which puts a typical
    // ~8 KB server URI limit at roughly 600 projects before OpenProject answers
    // 414 instead of truncating. Deliberately not engineered around: a single
    // operator in 600 projects is not a shape this system targets, and chunking
    // would add a merge/dedup path with its own failure modes. The same applies
    // to OpUserRepo::refreshMembership's batched project filter.
