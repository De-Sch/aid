...
std::string halTicket(int lockVersion, const std::string& statusHref) {
    ...
    // Must match the fixture config's "typeCall": OpenProject stamps every
    // work_package with its type, and decodeWebhook's type gate drops any
    // payload whose type is not the configured call type.
    j["_links"]["type"]["href"] = "/api/v3/types/7";
    return j.dump();
}
...
// T3b — a webhook for a work package of some OTHER type (Task, Bug, …) is
// dropped outright. OpenProject webhooks every work package it is subscribed
// to, but the dashboard queries filter on `type = typeCall`, so upserting one
// would put a row on every member's board that their next reload cannot return.
// The drop is free: no OpenProject traffic at all, not even recipient
// resolution, because the gate sits ahead of the echo-grace delay.
TEST_F(QueryScope, WebhookForForeignWorkPackageTypeIsDroppedWithoutAnyTraffic) {
    MockHttpServer opSrv(opResponder);
    LoopThread lt;
    auto opPlugin = loadOpenProject(opSrv.port(), lt.loop());
    ASSERT_NE(opPlugin->get(), nullptr);
    ...
    FakeUiNotifier ui;
    auto foreign = nlohmann::json::parse(halTicket(1, "/api/v3/statuses/1"));
    foreign["_links"]["type"]["href"] = "/api/v3/types/1"; // config's typeCall is "7"
    const std::string payload =
        std::string{R"({"action":"work_package:updated","work_package":)"} + foreign.dump() + "}";

    auto result = runOnLoop<int>(lt.loop(), [&]() -> Task<Result<int>> {
        auto decoded = co_await store.decodeWebhook(payload);
        if (!decoded)
            co_return 0; // parse error
        if (!decoded->has_value())
            co_return 1; // dropped — the outcome under test
        TicketDeltaEmitter emitter{store, ui};
        (void)co_await emitter.emitTicketDelta(std::move((*decoded)->ticket));
        co_return 2;
    });
    ...
    ASSERT_TRUE(result.has_value())
        << (result.has_value() ? std::string{} : result.error().message);
    EXPECT_EQ(*result, 1) << "a non-call work package must decode to nullopt";
    EXPECT_TRUE(ui.ticketUpserts.empty()) << "and must never reach anyone's dashboard";
    EXPECT_TRUE(ui.ticketRemoves.empty());
    EXPECT_EQ(opSrv.count(isMemberships), 0U)
        << "the gate runs before recipient resolution — a foreign type costs one JSON parse";
    EXPECT_EQ(opSrv.count(isAnyWorkPackages), 0U);
}

// T3c — a call ticket whose status is outside the configured workflow
// ("On hold"): OpStatusMap degrades it to New, which would upsert a phantom row
// the dashboard query cannot return. decodeWebhook normalises the enum off the
// dashboard's two-status set, so the emitter pushes a ticket_remove instead.
TEST_F(QueryScope, WebhookForOutOfWorkflowStatusRemovesInsteadOfUpserting) {
    MockHttpServer opSrv(opResponder);
    LoopThread lt;
    auto opPlugin = loadOpenProject(opSrv.port(), lt.loop());
    ASSERT_NE(opPlugin->get(), nullptr);
    ...
    FakeUiNotifier ui;
    // statuses 1/2/3 are New/InProgress/Closed in the fixture config; 42 is not
    // configured at all.
    const std::string payload = std::string{R"({"action":"work_package:updated","work_package":)"} +
                                halTicket(1, "/api/v3/statuses/42") + "}";
    ...
    ASSERT_TRUE(result.has_value())
        << (result.has_value() ? std::string{} : result.error().message);
    EXPECT_EQ(*result, 1) << "a call ticket still decodes — only its dashboard eligibility changes";
    EXPECT_TRUE(ui.ticketUpserts.empty()) << "no phantom row the reload cannot reproduce";
    ASSERT_EQ(ui.ticketRemoves.size(), 1U) << "it must be taken off the recipient's board";
    EXPECT_EQ(std::get<1>(ui.ticketRemoves[0]).v, "100");
}
