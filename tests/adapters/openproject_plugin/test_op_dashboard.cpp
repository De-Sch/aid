...
// The end-to-end statement of the un-paged-GET bug. alice belongs to 25 projects;
// OpenProject serves them 20-then-5. Before projectsForUser was paginated it
// returned only the first 20, so arm A never asked about project 31 and its
// ticket vanished from the dashboard — silently, and identically on every reload.
TEST(OpDashboardBuilder, BuildSeesTicketsInProjectsPastTheFirstPage) {
    DashHarness h;

    auto projectsPage = [](int firstId, int count, long long total) {
        json j;
        j["_embedded"]["elements"] = json::array();
    ...
    h.dispatcher.enqueueResponse(200, usersOne("alice", 9));     // hrefFor(alice)
    h.dispatcher.enqueueResponse(200, projectsPage(11, 20, 25)); // projects page 1
    h.dispatcher.enqueueResponse(200, projectsPage(31, 5, 25));  // projects page 2
    // Arm A: the ticket lives in project 31, which only page 2 revealed.
    h.dispatcher.enqueueResponse(200, collectionOf({halCallTicket("7", "31")}));
    h.dispatcher.enqueueResponse(200, R"({"_embedded":{"elements":[]}})"); // arm B empty

    auto r = drainSync(h.builder.build(aid::UserHandle{"alice"}));
    ASSERT_TRUE(r.has_value()) << r.error().message;
    ASSERT_EQ(r->size(), 1U) << "a ticket in a page-2 project must reach the dashboard";
    EXPECT_EQ((*r)[0].id.v, "7");

    // Arm A's work_packages filter must name every one of the 25 projects — the
    // last one included, which is what proves both pages were folded in.
    ASSERT_GE(h.dispatcher.calls().size(), 4U);
    const auto& armA = h.dispatcher.calls()[3].path;
    EXPECT_NE(armA.find("%2235%22"), std::string::npos)
        << "project 35 (from page 2) must be in the arm-A filter";
}
