...
//   5 — decodeWebhook filters inbound webhooks down to the dashboard's own
//       predicate (work-package type == typeCall, and a status the dashboard
//       queries actually return). A contract-4 `.so` loads cleanly, exports the
//       same symbols and the same layout, yet keeps pushing every edited Task /
//       Bug / Feature onto every member's board as a row that disappears on the
//       next reload — a user-visible defect with no other guard against it.
//       Layout is unchanged (no value type gained a field), so the PluginAbiTag
//       deliberately does NOT move.
//
//   6 — every OpenProject HAL collection GET is paginated. OpUserRepo's
//       projectsForUser and projectMembers each issued ONE un-paged GET, and
//       OpenProject's default pageSize is 20 — so a viewer in >20 projects lost
//       every ticket in projects 21+ from their dashboard, and a project's 21st+
//       members received no live WS deltas. A contract-5 `.so` loads cleanly,
//       exports the same symbols and the same layout, and simply serves a
//       silently truncated dashboard: wrong data with no other guard against it,
//       the same reason levels 2 and 5 exist. Layout is unchanged (no value type
//       gained a field), so the PluginAbiTag deliberately does NOT move.
//
...
inline constexpr char kPluginContractTag[] = "AID_PLUGIN_CONTRACT=6";
