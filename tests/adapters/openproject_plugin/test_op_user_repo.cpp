...
// One page of a projects collection: ids [firstId, firstId+count) plus the
// authoritative HAL `total` the pagination loop stops on. `servedPageSize` is
// what OpenProject echoes back as the size it actually applied — a value LOWER
// than what we requested is the server clamping us, which must not be mistaken
// for "last page".
std::string projectsPage(int firstId, int count, long long total,
                         std::optional<int> servedPageSize = std::nullopt) {
    json j;
    j["_embedded"]["elements"] = json::array();
    for (int i = 0; i < count; ++i) {
        json p;
        p["id"] = firstId + i;
        p["_links"]["self"]["href"] = "/api/v3/projects/" + std::to_string(firstId + i);
        j["_embedded"]["elements"].push_back(std::move(p));
    }
    j["total"] = total;
    if (servedPageSize)
        j["pageSize"] = *servedPageSize;
    return j.dump();
}

// urlEncode(R"([["id","asc"]])") — the sortBy param paging.h emits for the two
// id-DESCENDING collections. Spelled out so the test fails if the encoding drifts.
constexpr const char* kSortByIdAsc = "sortBy=%5B%5B%22id%22%2C%22asc%22%5D%5D";

} // namespace

// The regression this whole change exists for: OpenProject's default pageSize is
// 20, so the old un-paged GET returned the viewer's first 20 projects and the
// dashboard silently lost every ticket in projects 21+.
TEST(OpUserRepo, ProjectsForUserPaginatesAcrossPages) {
    FakeHttpDispatcher d;
    FakeSleeper s;
    d.enqueueResponse(200, usersCollectionWith("alice", 9));
    d.enqueueResponse(200, projectsPage(/*firstId=*/11, /*count=*/20, /*total=*/25));
    d.enqueueResponse(200, projectsPage(/*firstId=*/31, /*count=*/5, /*total=*/25));
    ...
    auto ids = drainSync(users.projectsForUser(aid::UserHandle{"alice"}));
    ASSERT_TRUE(ids.has_value()) << ids.error().message;
    ASSERT_EQ(ids->size(), 25U) << "must page past the first 20 rows";
    EXPECT_EQ((*ids)[0].v, "11");
    EXPECT_EQ((*ids)[24].v, "35");

    ASSERT_GE(d.calls().size(), 3U);
    EXPECT_NE(d.calls()[1].path.find("offset=1"), std::string::npos);
    EXPECT_NE(d.calls()[2].path.find("offset=2"), std::string::npos);
}

TEST(OpUserRepo, ProjectsForUserHonoursTotalEvenWhenServerClampsPageSize) {
    FakeHttpDispatcher d;
    FakeSleeper s;
    d.enqueueResponse(200, usersCollectionWith("alice", 9));
    // We ask for 1000; the server clamps to 100 and says so. Stopping on "page
    // shorter than requested" would truncate at 100 of 250.
    d.enqueueResponse(200, projectsPage(1, 100, 250, /*servedPageSize=*/100));
    d.enqueueResponse(200, projectsPage(101, 100, 250, /*servedPageSize=*/100));
    d.enqueueResponse(200, projectsPage(201, 50, 250, /*servedPageSize=*/100));
    ...
    auto ids = drainSync(users.projectsForUser(aid::UserHandle{"alice"}));
    ASSERT_TRUE(ids.has_value()) << ids.error().message;
    EXPECT_EQ(ids->size(), 250U) << "must page by total, not by the requested pageSize";
    ASSERT_GE(d.calls().size(), 4U);
    EXPECT_NE(d.calls()[3].path.find("offset=3"), std::string::npos);
}

TEST(OpUserRepo, ProjectsForUserRequestsExplicitPageSizeAndSortById) {
    FakeHttpDispatcher d;
    FakeSleeper s;
    d.enqueueResponse(200, usersCollectionWith("alice", 9));
    d.enqueueResponse(200, projectsPage(11, 1, 1));
    ...
    ASSERT_GE(d.calls().size(), 2U);
    const auto& path = d.calls()[1].path;
    // An explicit pageSize is the actual fix — omitting it is what got us 20.
    EXPECT_NE(path.find("pageSize=1000"), std::string::npos);
    // /api/v3/projects defaults to id-DESC, where a project created mid-walk
    // shifts a row across a page boundary and drops it.
    EXPECT_NE(path.find(kSortByIdAsc), std::string::npos);
}
...
// One page of a per-project memberships collection: principals
// /api/v3/users/[firstId, firstId+count), plus the authoritative `total`.
std::string membershipsPage(int firstId, int count, long long total) {
    json j;
    j["_embedded"]["elements"] = json::array();
    for (int i = 0; i < count; ++i) {
        json one;
        one["_links"]["principal"]["href"] = "/api/v3/users/" + std::to_string(firstId + i);
        j["_embedded"]["elements"].push_back(std::move(one));
    }
    j["total"] = total;
    return j.dump();
...
// The symmetric half of the projectsForUser cap: a project's 21st+ members were
// invisible to recipientsFor, so they got no live WS delta for any of its tickets.
TEST(OpUserRepo, ProjectMembersPaginatesAcrossPages) {
    FakeHttpDispatcher d;
    FakeSleeper s;
    d.enqueueResponse(200, membershipsPage(/*firstId=*/1, /*count=*/20, /*total=*/25));
    d.enqueueResponse(200, membershipsPage(/*firstId=*/21, /*count=*/5, /*total=*/25));
    for (int id = 1; id <= 25; ++id)
        d.enqueueResponse(200, userResource("user" + std::to_string(id), id));
    ...
    auto members = drainSync(users.projectMembers(aid::ProjectId{"11"}));
    ASSERT_TRUE(members.has_value()) << members.error().message;
    ASSERT_EQ(members->size(), 25U) << "must page past the first 20 memberships";
    EXPECT_EQ((*members)[0].v, "user1");
    EXPECT_EQ((*members)[24].v, "user25");

    // Two collection GETs (offset=1, offset=2) then one resolve per principal.
    ASSERT_GE(d.calls().size(), 2U);
    EXPECT_NE(d.calls()[0].path.find("offset=1"), std::string::npos);
    EXPECT_NE(d.calls()[1].path.find("offset=2"), std::string::npos);

    // Still exactly one cache entry — a second call must make no HTTP call at all.
    const std::size_t after = d.calls().size();
    auto again = drainSync(users.projectMembers(aid::ProjectId{"11"}));
    ASSERT_TRUE(again.has_value());
    EXPECT_EQ(again->size(), 25U);
    EXPECT_EQ(d.calls().size(), after) << "cached project must not re-fetch";
}

TEST(OpUserRepo, ProjectMembersRequestsExplicitPageSizeAndSortById) {
    FakeHttpDispatcher d;
    FakeSleeper s;
    d.enqueueResponse(200, membershipsPage(1, 1, 1));
    d.enqueueResponse(200, userResource("user1", 1));
    ...
    ASSERT_GE(d.calls().size(), 1U);
    const auto& path = d.calls()[0].path;
    EXPECT_NE(path.find("pageSize=200"), std::string::npos);
    EXPECT_NE(path.find(kSortByIdAsc), std::string::npos);
}
...
TEST(OpUserRepo, RefreshMembershipPaginatesAcrossPages) {
    ...
    // This loop moved into the shared pagedGet helper; its URL must not have
    // drifted. No sortBy here (unlike projectMembers): the poll re-issues this
    // query every 30 s, so a row displaced by a concurrent insert is picked up on
    // the next tick rather than persisting.
    EXPECT_EQ(d.calls()[before].path.find("sortBy"), std::string::npos);
    EXPECT_NE(d.calls()[before].path.find("pageSize=200"), std::string::npos);
}
