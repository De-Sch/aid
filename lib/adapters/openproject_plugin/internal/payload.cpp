...
std::string halTypeId(const nlohmann::json& hal) {
    auto linksIt = hal.find("_links");
    if (linksIt == hal.end() || !linksIt->is_object()) {
        return {};
    }
    auto typeIt = linksIt->find("type");
    if (typeIt == linksIt->end() || !typeIt->is_object()) {
        return {};
    }
    auto hrefIt = typeIt->find("href");
    if (hrefIt == typeIt->end() || !hrefIt->is_string()) {
        return {};
    }
    return hrefTail(hrefIt->get<std::string>());
}

bool isDashboardStatus(const aid::StatusId& id, const aid::crosscutting::TicketSystemConfig& cfg) {
    // An empty id can only come from a HAL body without a parseable status
    // href, which parseFromHal already rejects — treat it as not-eligible
    // rather than accidentally matching an operator's empty config value.
    if (id.v.empty()) {
        return false;
    }
    return id.v == cfg.statusNew.v || id.v == cfg.statusInProgress.v;
}
