...
TEST_F(AppendCommentTest, HappyPath_AppendsTrimmedTextOnNextLine_SavesAndNotifies) {
...
// First comment on a ticket: no leading separator. The description starts with
// the comment itself, not with a stray break.
TEST_F(AppendCommentTest, EmptyDescription_NoLeadingSeparator) {
    auto t = makeTicket(id(), "");
    ts_.nextFetchById.push_back(t);
    ts_.nextSave.push_back(t);
    ts_.nextRecipientsFor.push_back(
        Result<std::vector<UserHandle>>{std::vector<UserHandle>{viewer()}});
    ...
    auto uc = makeUseCase();
    auto r = sync(uc.run(id(), "first comment", viewer()));

    ASSERT_TRUE(r.has_value());
    EXPECT_TRUE(r->ok);
    ASSERT_EQ(ts_.saved.size(), 1U);
    EXPECT_EQ(ts_.saved[0].description, "first comment");
}

// What the operator typed must reach OpenProject as typed. Single newlines pass
// through (OpenProject renders them as breaks); a blank line becomes the "<br>"
// paragraph that survives as a real, cursor-placeable empty line.
TEST_F(AppendCommentTest, MultiLineComment_BlankLineBecomesEmptyLineParagraph) {
    auto t = makeTicket(id(), "");
    ts_.nextFetchById.push_back(t);
    ts_.nextSave.push_back(t);
    ts_.nextRecipientsFor.push_back(
    ...
    auto uc = makeUseCase();
    auto r = sync(uc.run(id(), "one\ntwo\n\nfour", viewer()));

    ASSERT_TRUE(r.has_value());
    EXPECT_TRUE(r->ok);
    ASSERT_EQ(ts_.saved.size(), 1U);
    EXPECT_EQ(ts_.saved[0].description, "one\ntwo\n\n<br>\n\nfour");
}
