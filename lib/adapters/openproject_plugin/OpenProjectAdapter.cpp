...
OpenProjectAdapter::decodeWebhook(std::string payload) {
    ...
    // Type gate. The ticket system webhooks EVERY work package it is subscribed
    // to, but the dashboard's own queries filter on `type = typeCall`, so an
    // edit to a Task/Bug/Feature would otherwise be upserted live onto every
    // project member's board and then vanish on their next reload (the snapshot
    // cannot return it). Drop it silently through the same nullopt path the
    // self-echo check below uses — deliberately BEFORE the grace delay, so a
    // foreign work package costs one JSON parse and nothing else: no sleep, no
    // membership lookup, no handler-ledger write, no fan-out. A payload with no
    // `_links.type` counts as foreign (every real work_package body carries it).
    if (halTypeId(*wp) != opCfg_.typeCall) {
        co_return std::optional<aid::WebhookDecode>{};
    }

    // Status normalisation, same "only deliver what a reload would return"
    // rule on the other axis. OpStatusMap::statusFor degrades any unconfigured
    // status to New, so a call ticket parked in an out-of-workflow status
    // ("On hold") would be upserted as a phantom New row that the dashboard
    // query — which filters on the exact configured status hrefs — cannot
    // return. Forcing the enum off the dashboard's two-status set makes the
    // consumer's existing New/InProgress→upsert, else→remove branch push a
    // ticket_remove instead. statusId stays verbatim, and ticket_remove carries
    // only (id, lockVersion), so the substituted enum never reaches the UI.
    if (!isDashboardStatus(ticket.statusId, opCfg_)) {
        ticket.status = aid::TicketStatus::Closed;
    }
