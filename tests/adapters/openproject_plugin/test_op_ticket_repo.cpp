...
TEST(OpTicketRepo, GetAllPagedHonoursTotalEvenWhenServerClampsPageSize) {
    ...
    // This loop moved into the shared pagedGet helper (paging.h). The work_packages
    // URLs deliberately gained NO sortBy — they are live-verified as they stand and
    // OpDashboardBuilder re-sorts the merged result anyway.
    EXPECT_EQ(h.dispatcher.calls()[0].path.find("sortBy"), std::string::npos);
}
