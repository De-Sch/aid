...
int main(int argc, char** argv) {
    ...
    // TEMP: the WAL is unwired from the controllers, so it must not create an
    // inbox.log either. walPath stays configured (and preflight still checks its
    // directory) so the path plumbing survives for the later fix-or-remove work.
    Wal wal{walPath.string(), clock, Wal::Backing::None};
    ...
    if (webhookCfg->has_value()) {
        // TEMP: same as the inbox WAL above — unwired, so no webhook.log either.
        webhookWal.emplace(webhookWalPath.string(), clock, Wal::Backing::None);
    ...
    if (auto uiCfg = cfg->ui(); uiCfg && uiCfg->documentRoot) {
        ...
        // Keep every static file in memory for the process lifetime. Drogon's
        // scale reads backwards from the obvious: 0 means "always cached",
        // NEGATIVE means "never cached" (HttpAppFramework.h; StaticFileRouter
        // caches whenever cacheTime >= 0, and CacheMap::insert with timeout 0
        // installs no expiry timer). This is deliberate, not a typo for -1.
        //
        // It is safe because docRoot only ever changes across a restart:
        // install.sh's upgrade path rebuilds the UI, swaps $UI_DIR atomically
        // and then `systemctl restart aid-daemon`; scripts/deploy.sh does the
        // same on a dev box. The bundle is ~800 KB over one IO thread, so the
        // cache costs nothing and saves a disk read on every request.
        //
        // Browser staleness is a SEPARATE mechanism and is already handled:
        // static responses go out with an epoch `Expires` and no max-age, so a
        // deploy is never served old JS out of the browser cache.
        //
        // Consequence when hacking on the UI: after `pnpm build` you must
        // restart the daemon. Otherwise the cached index.html keeps naming
        // hashed chunks that no longer exist on disk; they 404 into the
        // setDefaultHandler below, which answers index.html as text/html, the
        // browser refuses every module for its MIME type, and the dashboard is
