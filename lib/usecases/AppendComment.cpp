...
Task<Result<ActionResult>> AppendComment::run(aid::TicketId id, std::string text,
    ...
    // OpenProject renders a bare "\n" as a line break already, so the typed text
    // goes in as typed. The one thing markdown cannot spell is an EMPTY line,
    // which CommentText turns into the "<br>" paragraph OpenProject's own editor
    // uses — otherwise a double Enter would collapse into mere paragraph spacing
    // that the operator cannot put a cursor into. Applied here, on freshly typed
    // input only: `t.description` comes back from OpenProject verbatim, so
    // re-running the conversion on it would nest the markers it already holds.
    const std::string comment = aid::domain::CommentText::toMarkdown(trimmed);
    const aid::ports::TicketReducer reducer = [comment](aid::Ticket t) {
        // New comment starts on the next line. Skipped when the description is
        // empty so the first comment does not land after a stray leading break.
