...
// Requested page sizes for this repo's two collection queries. Both are pure
// performance knobs: pagedGet terminates on the collection's `total`, so even a
// server that clamps these lower just pages more times and still fetches
// everything. What matters is that they are SET AT ALL — OpenProject's default
// when the parameter is omitted is 20, which is precisely the silent truncation
// these constants exist to prevent.
constexpr int kProjectPageSize = 1000;   // /api/v3/projects rows are small
constexpr int kMembershipPageSize = 200; // /api/v3/memberships

// The project id carried by one /api/v3/projects HAL element: prefer the
// top-level "id" (HAL exposes it), fall back to the _links.self.href tail.
// nullopt when the element carries neither.
std::optional<aid::ProjectId> projectIdOf(const nlohmann::json& p) {
    if (auto idIt = p.find("id"); idIt != p.end()) {
        if (idIt->is_number_integer())
            return aid::ProjectId{std::to_string(idIt->get<long long>())};
        if (idIt->is_string())
            return aid::ProjectId{idIt->get<std::string>()};
    }
    if (auto links = p.find("_links"); links != p.end() && links->is_object()) {
        if (auto self = links->find("self"); self != links->end() && self->is_object()) {
            if (auto hr = self->find("href"); hr != self->end() && hr->is_string())
                return aid::ProjectId{hrefTail(hr->get<std::string>())};
        }
    }
    return std::nullopt;
}
...
Task<Result<std::vector<aid::ProjectId>>> OpUserRepo::projectsForUser(aid::UserHandle user) {
    ...
    // PAGED (was a single un-paged GET): OpenProject's default pageSize is 20, so
    // this used to return only the viewer's first 20 projects — and the dashboard's
    // member arm then lost EVERY ticket in projects 21+, deterministically, with no
    // self-heal across reloads. sortBy id-asc because /api/v3/projects defaults to
    // id-DESCENDING, where a project created mid-walk would shift a row across a
    // page boundary and drop it (see paging.h).
    std::string url = singleFilterUrl("/api/v3/projects", "principal", "=", hrefTail(*href));
    url.append(sortByIdAscParam());

    std::vector<aid::ProjectId> ids;
    // Missing/!array `_embedded.elements` still yields an empty vector rather than
    // an error: pagedGet counts zero rows for such a page and stops.
    auto sunk = co_await pagedGet(http_, std::move(url), kProjectPageSize,
                                  [&ids](const nlohmann::json& elements) -> Result<void> {
                                      ids.reserve(ids.size() + elements.size());
                                      for (const auto& p : elements) {
                                          if (auto id = projectIdOf(p))
...
Task<Result<std::vector<aid::UserHandle>>> OpUserRepo::projectMembers(aid::ProjectId project) {
    ...
    // PAGED (was a single un-paged GET): OpenProject's default pageSize is 20, so a
    // project's 21st+ members were invisible here — they received no live WS delta
    // for any ticket in it. The 30 s poll then made the cause hard to see rather
    // than fixing it: refreshMembership pages correctly, so it diffed a COMPLETE
    // fresh set against this TRUNCATED cached one and emitted the missing members
    // as spurious "added" ~30 s later, silently healing the cache. sortBy id-asc
    // because /api/v3/memberships defaults to id-DESCENDING (see paging.h).
    std::string url = singleFilterUrl("/api/v3/memberships", "project", "=", project.v);
    url.append(sortByIdAscParam());

    // Collect principal hrefs first so no json iterator is held across the
    // per-principal co_await below. The sink runs synchronously inside pagedGet
    // (it must not co_await), which is exactly what preserves that discipline:
    // every href is copied out before the resolve loop starts.
    std::vector<std::string> principalHrefs;
    auto sunk = co_await pagedGet(
        http_, std::move(url), kMembershipPageSize,
        [&principalHrefs](const nlohmann::json& elements) -> Result<void> {
            principalHrefs.reserve(principalHrefs.size() + elements.size());
...
Task<Result<std::vector<aid::MembershipDelta>>> OpUserRepo::refreshMembership() {
    ...
    // No sortBy: this URL is live-verified as it stands, and unlike the
    // projectMembers/projectsForUser queries it is re-issued every 30 s, so a row
    // displaced across a page boundary by a concurrent insert is picked up on the
    // very next tick rather than persisting.
    std::string base = multiFilterUrl("/api/v3/memberships", filters);

    // Page through the collection, accumulating (project, principal-href) rows.
    // Each membership names its project and principal by href only. The sink runs
    // synchronously inside pagedGet, so no json iterator is held across the
    // per-principal co_await in the resolve loop further down.
    struct Row {
    ...
    std::vector<Row> rows;
    auto sunk = co_await pagedGet(
        http_, std::move(base), kMembershipPageSize,
        [&rows](const nlohmann::json& elements) -> Result<void> {
            for (const auto& el : elements) {
                auto links = el.find("_links");
                if (links == el.end() || !links->is_object())
                    continue;
    ...
    if (!sunk)
        // SAFETY GUARD: a failed fetch ⇒ NO change. Keep every cached entry and
        // emit no delta — never read a transport/HTTP error as "all members
        // removed". (pagedGet propagates the error from whichever page failed;
        // partial `rows` from earlier pages are discarded with it, so a mid-walk
        // failure can never be mistaken for a shrunken membership either.)
        co_return std::vector<aid::MembershipDelta>{};
