...
public:
    ...
    // The COMPLETE list of projects the user belongs to — implementations must
    // paginate their upstream query rather than returning whatever one request
    // happened to fit. A truncated list here silently removes whole projects'
    // worth of tickets from the dashboard, with no error anywhere to trace.
