...
class Wal {
public:
    // TEMP: Backing::None builds a Wal that never touches the filesystem — no
    // parent directory, no backing file, no replay scan. It exists so the
    // daemon can run with the WAL unwired from the controllers without leaving
    // empty inbox.log / webhook.log behind. Every other caller (including all
    // tests) uses the defaulted Backing::File and is unaffected.
    enum class Backing { File, None };

    Wal(std::string path, aid::crosscutting::Clock& clock, Backing backing = Backing::File);
