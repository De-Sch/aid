...
void CallController::handlePost(const drogon::HttpRequestPtr& req,
    ...
    // TEMP: WAL not wired — events dispatch without durable logging.
    constexpr std::uint64_t kWalSeq = 0;

    const auto callid = aid::callidOf(*eventOpt);
    auto enq = mailbox_.enqueue(callid, std::move(*eventOpt), cidStr, kWalSeq);
    if (!enq) {
    ...
    logger_.debug("CallController: accepted callid=" + callid.v + " seq=" + std::to_string(kWalSeq),
                  LogType::BACKEND, std::string_view{cidStr});
