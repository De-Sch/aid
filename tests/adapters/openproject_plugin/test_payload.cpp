...
using aid::adapters::openproject::halTypeId;
using aid::adapters::openproject::isDashboardStatus;
...
// ─── halTypeId ────────────────────────────────────────────────────────────
//
// The webhook type gate (OpenProjectAdapter::decodeWebhook) is the only
// consumer: it compares halTypeId(work_package) against cfg.typeCall and drops
// the webhook when they differ, so an edited Task/Bug never lands on a
// dashboard the reload query would not return it to.

TEST(HalTypeId, ReadsTrailingIdOfTypeHref) {
    EXPECT_EQ(halTypeId(halBody()), "7");
    // …and that is exactly what the gate compares against.
    EXPECT_EQ(halTypeId(halBody()), sampleCfg().typeCall);
}

TEST(HalTypeId, ReadsTheWorkPackageObjectOutOfAWebhookEnvelope) {
    // The real envelope is {"action":…,"work_package":{<HAL>}}; decodeWebhook
    // narrows to the work_package object before calling us.
    const json env = {{"action", "work_package:updated"}, {"work_package", halBody()}};
    EXPECT_EQ(halTypeId(env["work_package"]), "7");
    // The envelope itself has no _links — asking it directly yields nothing,
    // which the gate treats as "not a call ticket".
    EXPECT_EQ(halTypeId(env), "");
}

TEST(HalTypeId, MultiSegmentHrefStillYieldsTheTail) {
    auto hal = halBody();
    hal["_links"]["type"]["href"] = "/api/v3/types/7/schema";
    EXPECT_EQ(halTypeId(hal), "schema");
}

TEST(HalTypeId, MissingOrMistypedPiecesYieldEmptyString) {
    auto hal = halBody();
    hal["_links"].erase("type");
    EXPECT_EQ(halTypeId(hal), "") << "no _links.type";

    hal = halBody();
    hal.erase("_links");
    EXPECT_EQ(halTypeId(hal), "") << "no _links at all";

    hal = halBody();
    hal["_links"]["type"] = "/api/v3/types/7";
    EXPECT_EQ(halTypeId(hal), "") << "type is a string, not a {href:…} object";

    hal = halBody();
    hal["_links"]["type"].erase("href");
    EXPECT_EQ(halTypeId(hal), "") << "type object without href";

    hal = halBody();
    hal["_links"]["type"]["href"] = 7;
    EXPECT_EQ(halTypeId(hal), "") << "non-string href";
}

TEST(HalTypeId, ForeignTypeDiffersFromTheConfiguredCallType) {
    auto hal = halBody();
    hal["_links"]["type"]["href"] = "/api/v3/types/1"; // e.g. Task
    EXPECT_NE(halTypeId(hal), sampleCfg().typeCall);
}

// ─── isDashboardStatus ────────────────────────────────────────────────────

TEST(IsDashboardStatus, TrueOnlyForTheTwoStatusesTheDashboardQueriesReturn) {
    const auto cfg = sampleCfg();
    EXPECT_TRUE(isDashboardStatus(aid::StatusId{"1"}, cfg));   // New
    EXPECT_TRUE(isDashboardStatus(aid::StatusId{"2"}, cfg));   // In Progress
    EXPECT_FALSE(isDashboardStatus(aid::StatusId{"3"}, cfg));  // Closed
    EXPECT_FALSE(isDashboardStatus(aid::StatusId{"42"}, cfg)); // unconfigured, e.g. "On hold"
    EXPECT_FALSE(isDashboardStatus(aid::StatusId{}, cfg));     // empty id
}

TEST(IsDashboardStatus, RejectsAnOutOfWorkflowStatusThatStatusForDegradedToNew) {
    // The reason this predicate reads the RAW id: statusFor() collapses every
    // unconfigured status to New, so a ticket parked in "On hold" would look
    // dashboard-eligible on the enum alone and be upserted as a phantom row
    // the reload query cannot return.
    const auto cfg = sampleCfg();
    const auto m = OpStatusMap::fromConfig(cfg);
    auto hal = halBody();
    hal["_links"]["status"]["href"] = "/api/v3/statuses/42";

    auto r = parseFromHal(hal, sampleFields(), m);
    ASSERT_TRUE(r.has_value()) << r.error().message;
    EXPECT_EQ(r->status, aid::TicketStatus::New); // degraded by statusFor…
    EXPECT_EQ(r->statusId.v, "42");               // …but the raw id survives
    EXPECT_FALSE(isDashboardStatus(r->statusId, cfg));
}
