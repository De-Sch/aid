...
Wal::Wal(std::string path, aid::crosscutting::Clock& clock, Backing backing)
    : path_(std::move(path)), clock_(clock) {
    // TEMP: WAL unwired from the controllers — Backing::None must create
    // nothing on disk. Leaving fd_ at -1 and pendingReplay_ empty is safe here
    // because append() is no longer called and ack() only ever sees seq 0,
    // which short-circuits at the ackedPrefix_ check before any rewrite.
